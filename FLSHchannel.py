...
plot_var = getvar(ncfile, "FLSHP", timeidx=timeidx)
...
flash_data = np.sum(to_np(plot_var), axis=0)
ax.scatter(lons[flash_data > 0],lats[flash_data > 0],s=25,marker="X",c='red',transform=crs.PlateCarree())
...
# Add the gridlines
plt.title(f"Positive Flash Channel Points at {date_format}",fontsize="14")
plt.show()

plt.savefig(savepath+f"FLASHchannel{wrf_date_time.year:04d}{wrf_date_time.month:02d}{wrf_date_time.day:02d}{wrf_date_time.hour:02d}{wrf_date_time.minute:02d}D{domain}T{timeidx}A{ATTEMPT}.png")
